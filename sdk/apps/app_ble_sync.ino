...
void appNameClass::drawSyncIconAnimation(bool draw, bool onlyBgDraw){
    #define MIN_K_PREVALUE 10
    #define MAX_K_PREVALUE 511

    uint16_t k = (millis()/3)%MAX_K_PREVALUE + 1;
    ...
    unsigned char icon_circle_blue = 0xff; // 0xff

    if(k<=255){
        icon_circle_green = (unsigned char)((uint16_t)0x66 * k / 255); // 0x66
        icon_circle_blue = (unsigned char)((uint16_t)0xff * k / 255); // 0xff
    }else{
        icon_circle_green = (unsigned char)((uint16_t)0x66 * (MAX_K_PREVALUE-k) / 255); // 0x66
        icon_circle_blue = (unsigned char)((uint16_t)0xff * (MAX_K_PREVALUE-k) / 255); // 0xff
    }
    

    //debug("K: " + String((uint16_t)0xff * k / 255));
    //delay(1000);
    if(onlyBgDraw){
...
void appNameClass::drawActivity(bool draw, byte activity){
    ...
    if(this->currentActivity==ACTIVITY_CONNECT_TO_CHARGE){
            ...
            /*
            const unsigned char sync_process_icon[] = {
                    ,
            ...
            if(draw){
                ...
                drawSyncIconAnimation(draw, false);// drawImage(draw, sync_process_icon, SCREEN_WIDTH/2 - 12, STYLE_STATUSBAR_HEIGHT + 38);
...
void appNameClass::onLoop(){
    if(this->currentActivity==ACTIVITY_SYNCING){
        drawSyncIconAnimation(true, true);
    }
}
